...
{
  char *error_msg = NULL;
  CHECKCODERET(sqlite3_exec(sqlite3_descriptor, stmt_string, 0, 0, &error_msg) == SQLITE_OK,
			   false,
...
bool add_user(sqlite3 *sqlite3_descriptor, const char *username, const char *password) {
  ...
  CHECKRET(sprintf(stmt_string, "INSERT INTO USERS VALUES('%s', '%s');", username, password) > 0, false,
		   "Error at 'sprintf()'")
  bool userExists = false;
  CHECKRET(user_exists(sqlite3_descriptor, username, &userExists) == true, false, "Error cannot check existence of user")
  if (userExists == false) {
...
bool add_permission(sqlite3 *sqlite3_descriptor, const char *repository_name, const char *username) {
  ...
  CHECKRET(permission_exists(sqlite3_descriptor, repository_name, username, &has_perms) == true, false,
  ...
  CHECKRET(sprintf(stmt_string, "INSERT INTO PERMISSIONS VALUES('%s', '%s');", repository_name, username) > 0, false,
		   "Error at 'sprintf()'")
  return run_non_select_sqlite_statement(sqlite3_descriptor, stmt_string);
}
...
bool add_access(sqlite3 *sqlite3_descriptor, const char *repository_name, const char *username) {
  ...
  CHECKRET(strlen(username) <= MAX_USER_NAME_LEN, false, "Username too big")
  bool has_perms = false;
  CHECKRET(access_exists(sqlite3_descriptor, repository_name, username, &has_perms) == true, false,
		   "Cannot check user access")
  if (has_perms == true) { return true; }
  char stmt_string[MAX_SQL_STMT_LEN + MAX_REPO_NAME_LEN + MAX_USER_NAME_LEN];
  CHECKRET(sprintf(stmt_string, "INSERT INTO ACCESS VALUES('%s', '%s');", repository_name, username) > 0, false,
		   "Error at 'sprintf()'")
  return run_non_select_sqlite_statement(sqlite3_descriptor, stmt_string);
}
...
bool add_version(sqlite3 *sqlite3_descriptor,
  ...
  CHECKRET(sprintf(stmt_string, "INSERT INTO VERSIONS VALUES('%s', '%s', %d, %lu, '%s');",
				   repository_name, author_username, version, unix_date, changelog) > 0, false,
		   "Error at 'sprintf()'")
  return run_non_select_sqlite_statement(sqlite3_descriptor, stmt_string);
}
...
bool add_repo(sqlite3 *sqlite3_descriptor, const char *repository_name, const char *username, u64 unix_date) {
  char stmt_string[MAX_SQL_STMT_LEN + MAX_REPO_NAME_LEN + MAX_USER_NAME_LEN + UNPREDICTED_LEN];
  CHECKRET(sprintf(stmt_string, "INSERT INTO REPOSITORY VALUES('%s', '%s', %lu, %d);", repository_name, username,
				   unix_date, 0) > 0, false, "Error at 'sprintf()'")
  return run_non_select_sqlite_statement(sqlite3_descriptor, stmt_string);
}
...
bool del_permission(sqlite3 *sqlite3_descriptor, const char *repository_name, const char *username) {
  ...
  CHECKRET(sprintf(stmt_string, "DELETE FROM PERMISSIONS WHERE repository_name='%s' AND username='%s';", repository_name,
				   username) > 0, false, "Error at 'sprintf()'")
  return run_non_select_sqlite_statement(sqlite3_descriptor, stmt_string);
}
...
bool del_access(sqlite3 *sqlite3_descriptor, const char *repository_name, const char *username) {
  ...
  char stmt_string[MAX_SQL_STMT_LEN + MAX_USER_NAME_LEN + MAX_REPO_NAME_LEN];
  CHECKRET(sprintf(stmt_string, "DELETE FROM ACCESS WHERE repository_name='%s' AND username='%s';", repository_name,
				   username) > 0, false, "Error at 'sprintf()'")
  return run_non_select_sqlite_statement(sqlite3_descriptor, stmt_string);
}
...
bool version_exists(sqlite3 *sqlite3_descriptor, const char *repository_name, u16 version, bool *context) {
  ...
  CHECKRET(sprintf(condition, "repository_name='%s' AND version=%d", repository_name, version) > 0, false,
		   "Error at 'sprintf()'")
  u32 count = 0;
...
bool repo_exists_in_REPOSITORY(sqlite3 *sqlite3_descriptor, const char *repository_name, bool *context) {
  ...
  CHECKRET(sprintf(condition, "repository_name='%s'", repository_name) > 0, false,
		   "Error at 'sprintf()'")
  u32 count = 0;
 ...
 * */
{
  ...
  CHECKCODERET(sprintf(stmt_string, "SELECT COUNT(*) FROM %s WHERE %s;", table_name, condition) > 0,
			   false,
			   { free(stmt_string); },
			   "Error at 'sprintf()'")
  sqlite3_stmt *stmt;
  CHECKCODERET(sqlite3_prepare_v2(sqlite3_descriptor, stmt_string, -1, &stmt, NULL) == SQLITE_OK,
			   false,
			   { free(stmt_string); },
			   "Error at preparing statement")
  CHECKCODERET(sqlite3_step(stmt) == SQLITE_ROW || sqlite3_step(stmt) == SQLITE_DONE,
			   false,
			   { free(stmt_string); },
...
bool get_current_version_number(sqlite3 *sqlite3_descriptor, const char *repository_name, u16 *context) {
  ...
  CHECKRET(sprintf(stmt_string, "SELECT version FROM versions WHERE repository_name='%s' ORDER BY version DESC LIMIT 1;",
				   repository_name) > 0, false, "Error at 'sprintf()'")
  sqlite3_stmt *stmt;
...
bool get_next_version_number(sqlite3 *sqlite3_descriptor, const char *repository_name, u16 *context) {
  ...
  CHECKRET(get_current_version_number(sqlite3_descriptor, repository_name, &current_version) == true, false,
		   "Error at getting the current version")
...
bool get_changelog_version(sqlite3 *sqlite3_descriptor, const char *repository_name, u16 version, char *context) {
  ...
  CHECKRET(
	  sprintf(stmt_string, "SELECT changelog FROM versions WHERE repository_name='%s' AND version=%d;", repository_name,
...
bool get_ls_remote_files_version(sqlite3 *sqlite3_descriptor,
								 const char *repository_name,
								 u16 version,
...
bool get_ls_deleted_files_version(sqlite3 *sqlite3_descriptor,
								  const char *repository_name,
								  u16 version,
								  JSON_Array *json_array) {
  char stmt_string[MAX_SQL_STMT_LEN + MAX_REPO_NAME_LEN + UNPREDICTED_LEN];
  char *error_msg = NULL;
  CHECKRET(sprintf(stmt_string, "SELECT filename FROM DELETIONS WHERE repository_name='%s' AND version=%d;",
				   repository_name, version) > 0, false, "Error at 'sprintf()'")
  CHECKCODERET(sqlite3_exec(sqlite3_descriptor, stmt_string, inserter, json_array, &error_msg) == SQLITE_OK, false, {
	fprintf(stderr, "%s", error_msg);
	sqlite3_free(stmt_string);
...
bool make_repo_public(sqlite3 *sqlite3_descriptor, const char *repository_name) {
  CHECKRET(strlen(repository_name) <= MAX_REPO_NAME_LEN, false, "Repository name too big")
  char stmt_string[MAX_SQL_STMT_LEN + MAX_REPO_NAME_LEN];
  CHECKRET(sprintf(stmt_string, "UPDATE REPOSITORY SET is_public=1 WHERE repository_name='%s';",
				   repository_name) > 0, false, "Error at 'sprintf()'")
  return run_non_select_sqlite_statement(sqlite3_descriptor, stmt_string);
}
...
bool make_repo_private(sqlite3 *sqlite3_descriptor, const char *repository_name) {
  CHECKRET(strlen(repository_name) <= MAX_REPO_NAME_LEN, false, "Repository name too big")
  char stmt_string[MAX_SQL_STMT_LEN + MAX_REPO_NAME_LEN];
  CHECKRET(sprintf(stmt_string, "UPDATE REPOSITORY SET is_public=0 WHERE repository_name='%s';",
				   repository_name) > 0, false, "Error at 'sprintf()'")
  return run_non_select_sqlite_statement(sqlite3_descriptor, stmt_string);
}